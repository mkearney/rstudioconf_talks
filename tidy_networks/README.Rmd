...
+ [thomasp85/tidygraph](https://github.com/thomasp85/tidygraph): 
...
+ `tidygraph::activate()`: Determine the context of subsequent manipulations As a tbl_graph can be considered as a collection of two linked tables it is
...
+ `tidygraph::bind_edges()`: Add graphs, nodes, or edges to a tbl_graph These functions are tbl_graph pendants to dplyr::bind_rows() that allows
...
+ `tidygraph::bind_nodes()`: Add graphs, nodes, or edges to a tbl_graph These functions are tbl_graph pendants to dplyr::bind_rows() that allows
you to grow your tbl_graph by adding rows to either the nodes data, the
edges data, or both. As with bind_rows() columns are matched by name and
are automatically filled with NA if the column doesn't exist in some
instances. In the case of bind_graphs() the graphs are automatically
...
+ `tidygraph::bind_graphs()`: Add graphs, nodes, or edges to a tbl_graph These functions are tbl_graph pendants to dplyr::bind_rows() that allows
you to grow your tbl_graph by adding rows to either the nodes data, the
edges data, or both. As with bind_rows() columns are matched by name and
are automatically filled with NA if the column doesn't exist in some
instances. In the case of bind_graphs() the graphs are automatically
...
+ `ggraph::ggraph()`: Create a ggraph plot This function is the equivalent of ggplot in ggplot2.
...
+ `ggraph::geom_node_point()`: Show nodes as points This geom is equivalent in functionality to geom_point
and allows for simple plotting of nodes in different shapes, colours and sizes.


+ `ggraph::geom_edge_link()`: Draw edges as straight lines between nodes This geom draws edges in the simplest way - as straight lines between the
